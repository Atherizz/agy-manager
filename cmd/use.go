...
var useCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if !profile.Exists(p, name) { return fmt.Errorf("%s Profile not found", errorStyle.Render("✗")) }

        state, _ := profile.LoadState(p)

        if state.ActiveProfile == name { return nil }
        ...
        if err := profile.SwapToProfile(p, name); err != nil { return err }
